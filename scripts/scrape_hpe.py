...
def scrape(cfg: dict) -> List[Update]:
    """Verejne rozhrani scraperu - `cfg` je cely obsah config/devices/hpe.yaml
    (klice 'families' a 'generations'). Stejny podpis `scrape(cfg)` maji
    vsechny scrape_<vyrobce>.py moduly, aby si je run_all.py mohl sam najit
    a zavolat, aniz by je musel jmenovite znat (viz run_all.discover_vendors)."""
    return _scrape(cfg.get("families", []), cfg.get("generations", []))


def _scrape(families: List[str], generations: List[str]) -> List[Update]:
    updates: List[Update] = []
...
if __name__ == "__main__":
    ...
    res = scrape({"families": ["DL380", "DL360"], "generations": ["Gen11", "Gen12"]})
