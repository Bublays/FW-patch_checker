...
def _matches_model(component: ET.Element, models: Iterable[str]) -> str | None:
    ...
    Modelove cislo (napr. "R740") musi v Display nazvu byt cele slovo - ne jen
    podretezec - aby se "PowerEdge R740" neshodovalo omylem i s "PowerEdge
    R740xd" nebo "PowerEdge R740xd2" (ktere nejsou v config/devices.yaml)."""
    ...
    for brand in supported.findall("Brand"):
        for model_el in brand.findall("Model"):
            ...
            for wanted in models:
                model_number = wanted.split()[-1]  # napr. "PowerEdge R740" -> "R740"
                pattern = r"(?<![A-Za-z0-9])" + re.escape(model_number) + r"(?![A-Za-z0-9])"
...
def scrape(cfg: dict) -> List[Update]:
    """Verejne rozhrani scraperu - `cfg` je cely obsah config/devices/dell.yaml
    (klic 'models'). Stejny podpis `scrape(cfg)` maji vsechny scrape_<vyrobce>.py
    moduly, aby si je run_all.py mohl sam najit a zavolat."""
    return _scrape(cfg.get("models", []))


def _scrape(models: List[str]) -> List[Update]:
    updates: List[Update] = []
    try:
...
if __name__ == "__main__":
    ...
    res = scrape({"models": ["PowerEdge R440", "PowerEdge R740", "PowerEdge T140"]})
