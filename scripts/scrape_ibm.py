...
def scrape(cfg: dict) -> List[Update]:
    """Verejne rozhrani scraperu - `cfg` je cely obsah config/devices/ibm.yaml
    (klice 'flashsystem', 'tape_fixlist', 'tape_recommendation'). Stejny
    podpis `scrape(cfg)` maji vsechny scrape_<vyrobce>.py moduly, aby si je
    run_all.py mohl sam najit a zavolat."""
    return _scrape(
        cfg.get("flashsystem", []),
        cfg.get("tape_fixlist", []),
        cfg.get("tape_recommendation", []),
    )
